...
/**
 * This class represents a service for translating text using Google Cloud
 * Translation API.
 */
@Service
public class TranslatorService extends AIServiceBase {
  ...
  /**
   * This method translates a text to a target language using Google Cloud
   * Translation API.
   *
  ...
  /**
   * Retrieves the list of supported languages from the Google Cloud Translation
   * API.
   *
  ...
  public List<String> getSupportedLanguages() throws IOException {
    ...
    try (TranslationServiceClient client = TranslationServiceClient.create()) {
      LocationName parent = LocationName.of(projectId, GLOBAL);
      GetSupportedLanguagesRequest request = GetSupportedLanguagesRequest.newBuilder().setParent(parent.toString())
          .build();
